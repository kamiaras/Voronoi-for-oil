...
def is_feasible(n: int, problem: dict, config: dict):
    """
    ...
    problem : dict
        ...
        - 'loss':    callable(added_pts: List[(name,(x,y))]) → float
    ...
    if n == 0:
        score = loss_fn([])  # loss_fn now only takes added points
        # +inf signals infeasible; otherwise score = –min_area
    ...
    if solver == "qpso_pairwise":
        new_pts = solvers.qpso_pairwise_solver(problem, n, params)
    elif solver == "ga_deap_solver":
        new_pts = solvers.ga_deap_solver(problem, n, params)
    elif solver == "de_scipy_solver":
        new_pts = solvers.de_scipy_solver(problem, n, params)
    else:
        raise NotImplementedError(f"Solver '{solver}' is not supported")

    # 3) Evaluate only the added points
    score = loss_fn(new_pts)
    if score == float("inf"):
